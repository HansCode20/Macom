...
import { getMangaDetail } from "../../utils/MangaApi";
      ...
      <div>
        <nav className="text-sm text-white mb-4 ">
          ...
          <Link to={`/manga/${id}`} className="hover:underline">
            {mangaDetail.name || "Loading..."}
          </Link>{" "}
          {/* Judul manga dari API */}
        </nav>
      </div>
          ...
          {Array.isArray(mangaDetail.chapterList) ? (
            mangaDetail.chapterList.map((chapter, index) => (
              <Link to={`/manga/${id}/${chapter.id}`}>
                <div
                  key={index}
                  className="relative group bg-[#433d8b] p-4 text-center rounded-lg truncate"
                >
                  {chapter.name}
