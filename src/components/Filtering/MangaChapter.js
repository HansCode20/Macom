...
import { getMangaPath } from "../../utils/Api";
...
const MangaChapter = () => {
    ...
    const currentChapterIndex = chapters.findIndex(chapter => chapter.id === ch);
    ...
    return (
        <div className="flex flex-col justify-center items-center space-y-20 scrollbar-custom">
            <div className={`fixed flex justify-center items-center top-0 p-4 z-20 w-full lg:w-1/3 bg-white/20 shadow-md rounded-lg ${scroll ? 'text-black opacity-25' : 'text-white'}`}>
                <nav className="text-xs lg:text-md md:text-sm">
                    <Link to="/" className="hover:underline">Macom</Link>
                    <span> &gt; </span>
                    <Link to={`/manga/${id}`} className="hover:underline">{mangaTitle}</Link>
                    <span> &gt; </span>
                    <Link to={`/manga/${id}/${ch}`} className="hover:underline">{chapterTitle}</Link>
                </nav>
            </div>

            <div className="flex flex-col gap-0 lg:gap-3 bg-black">
                {mangaImage && mangaImage.map((chapter, index) => (
                    <div key={index}>
                        <img src={chapter.image} alt={chapter.title} className="w-30 h-30 object-contain mx-auto" />
                    </div>
                ))}
            </div>
            ...
            <div className="flex justify-center items-center gap-4 ">
                <button
                 ...
                 className={`btn bg-[#2E236C] ${currentChapterIndex === 0 ? 'btn-disabled' : ''}`}
                >
                    Prev
                </button>
                ...
                <button
                    ...
                    className={`btn bg-[#2E236C] ${currentChapterIndex === chapters.length - 1 ? 'btn-disabled' : ''}`}
                >
                    Next
                </button>
            </div>
